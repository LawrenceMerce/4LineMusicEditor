# SATB Arranger - Version 3.1
# 卷帘可编辑（移动/拉伸/新增/删除/力度），延音轨可单独调整，规则检查，SF2/Sampler/System MIDI
# 交互优化：命中更稳、拉伸柄更宽、Ctrl=推挤模式、Shift=力度拖动、右键删除任意声部音符
# 注意：Windows + SoundFont 需要可用的 fluidsynth DLL（见 FLUIDSYNTH_BIN）
...
APP_TITLE   = "SATB Arranger — v3.1 (Editable Roll + Velocity + Pedal)"
DEFAULT_BPM = 90
DEFAULT_METER = (4, 4)
...
def parse_token(tok: str):
    ...
    if m2: return (note_to_midi(m2.group(1)), float(m2.group(2)))
...
class PianoRollCanvas(tk.Canvas):
    def __init__(self, master, on_modified, get_active_voice, set_active_voice, get_grid, **kw):
        ...
        # 交互状态
        self._hit=None       # 当前操作的音符信息
        self._drag_pedal=None # pedal 编辑状态
        self._selected=None   # (voice, index)
        self._resize_grip=10  # 拉伸柄宽度
        ...
        self.bind("<Shift-B1-Motion>", self.on_shift_drag)  # 力度
        self.bind("<Delete>", self.on_key_delete); self.focus_set()  # 让 Delete 生效

    # ---- 绘制 ----
    def set_score(self, sc:Score, sustain=None):
        ...
        for v in VOICE_ORDER:
            ...
            for idx,n in enumerate(sc.parts[v].notes):
                if n.pitch is not None and n.beats>0:
                    ...
                    # 拉伸柄
                    self.create_rectangle(x1-self._resize_grip, y0, x1, y1, fill="#eaeaea", outline="", tags=("grip", v))
                    # 选中描边
    ...
    def _pedal_edge_hit(self, x, y):
        # 返回 (idx, edge)；edge ∈ {"L","R", None}，None 代表点在区段内部
    ...
    def on_left_down(self, ev):
        ...
        if hit:
            ...
            self._selected=(v,idx)
            self.set_active_voice(v)  # 自动切换编辑声部
            # 建立拖动状态
            ws=self._score.parts[v].with_starts()
            st, n = ws[idx]
            self._hit={"voice":v,"index":idx,"start":st,"dur":n.beats,"pitch":n.pitch,
                       "vel":n.vel,"mode":("resize" if hit["grip"] else "move"),
                       "x0":x,"y0":y}
            self.focus_set()  # 让 Delete 起作用
            return
    ...
    def on_left_drag(self, ev):
        ...
        if self._drag_pedal is not None:
            mode=self._drag_pedal["mode"]
            if mode=="resize":
                # 实时预览：修改该段端点
                i=self._drag_pedal["index"]; edge=self._drag_pedal["edge"]
                s,e=self._sustain[i]
                b=q(self.x_to_beat(x), grid)
        ...
        if info["mode"]=="move":
            ...
            # resize：Ctrl=推挤模式（允许超过 next_start 并把后面向后推）
            right=start+dur; new_right=q(max(self.x_to_beat(x), start+grid), grid)
            push = (ev.state & 0x0004) != 0  # Control 修饰键
    ...
    def on_shift_drag(self, ev):
        ...
        n.vel=new_vel
        self._score.parts[v].notes[idx]=n
        self.set_score(self._score, self._sustain)
    ...
    def _insert_note(self, v, beat, dur, pitch, vel):
        part=self._score.parts[v]; ws=part.with_starts(); total=self._score.total_beats()
        beat=q(beat, self.get_grid()); dur=q(dur, self.get_grid())
        # 找插入点：若命中现有音符则插在其尾部
        pos=0
        for i,(st,n) in enumerate(ws):
            if st<=beat<st+n.beats and n.pitch is not None: beat=st+n.beats; pos=i+1
            elif st<beat: pos=i+1
        new_notes=[]; cur=0.0; inserted=False
        for st,n in ws:
            if not inserted and beat<=st:
                if beat>cur: new_notes.append(Note(None, beat-cur))
                new_notes.append(Note(pitch, dur, vel)); cur=beat+dur; inserted=True
            if st>=cur: new_notes.append(Note(n.pitch, n.beats, n.vel)); cur=st+n.beats
        if not inserted:
            if beat>cur: new_notes.append(Note(None, beat-cur))
            new_notes.append(Note(pitch, dur, vel)); cur=beat+dur
        if cur<total: new_notes.append(Note(None, total-cur))
        part.notes=new_notes
        self.set_score(self._score, self._sustain)
    ...
    def _delete_note(self, v, idx):
        ...
        st,n=ws[idx]
        new_notes=[]; cur=0.0
        for i,(s,nn) in enumerate(ws):
            if i==idx:
                # 转为空白
                continue
            if s>cur: new_notes.append(Note(None, s-cur))
            new_notes.append(Note(nn.pitch, nn.beats, nn.vel)); cur=s+nn.beats
        if cur<total: new_notes.append(Note(None, total-cur))
        part.notes=new_notes
        self.set_score(self._score, self._sustain)
...
class App:
    ...
    def _build_ui(self):
        ...
        ttk.Label(top, text="  可编辑卷帘 · 力度 · 延音 · 规则检查", style="Caption.TLabel").pack(side="left")
        ...
        cbgrid.bind("<<ComboboxSelected>>", lambda e:self._set_grid(cbgrid.get()))
        ...
        roll_box=ttk.Labelframe(right, text="钢琴卷帘（拖动=移动；抓右侧=改时值；Ctrl+拉伸=推挤；双击空白=新增；右键=删除；Shift拖=力度；下方=延音）")
    ...
    def _set_grid(self, s): self.grid_res.set({"1/4":1.0,"1/8":0.5,"1/16":0.25}[s])
    ...
    def _score_from_texts(self, bpm=None):
        ...
        for v in VOICE_ORDER:
            ...
            seq=[]; 
            for t in toks:
                p,b=parse_token(t); seq.append(Note(p,b))
            sc.parts[v].extend(seq)
        sc.ensure_aligned()
        if self._current_score: sc.sustain = list(self._current_score.sustain)
        return sc

    def _texts_from_score(self, sc:Score):
        for v in VOICE_ORDER:
            ws=sc.parts[v].with_starts(); toks=[]
            for st,n in ws:
                if n.pitch is None: toks.append(f"R{dur_to_token(n.beats)}")
                else: toks.append(f"{format_pitch(n.pitch)}{dur_to_token(n.beats)}")
            self.texts[v].delete("1.0","end")
            total=0.0; out=[]
            for t in toks:
                out.append(t)
                # 简单分小节
                m=re.search(r"(\d+(?:\.\d+)?)$", t); d=float(m.group(1)) if m else DUR_MAP.get(t[-1],1.0)
                total+=d
        ...
        try: be.prepare()
            ...
            if not isinstance(be, SystemMIDIVanilla):
                try:
                    ...
                    self.status.set(f"后端失败，已降级 System MIDI：{e}")
    ...
    def export_midi(self):
        ...
        for i,v in enumerate(VOICE_ORDER):
            mf.addTempo(i, 0, sc.bpm); mf.addProgramChange(i, VOICE_TO_CH[v], 0, 0)
        for v in VOICE_ORDER:
            t=0.0
            for n in sc.parts[v].notes:
                ...
                t+=n.beats
        # 简化：MIDI 文件暂未写入 CC64；如需可扩展到轨道 0 插入控制器事件
    ...
    def on_score_modified(self, sc_from_roll: Score):
        # 只保留当前对象，不回写文本（避免丢失力度/踏板）；检查并刷新画布
        self._current_score=sc_from_roll
        issues=RuleChecker(sc_from_roll).check()
        for i in self.tree.get_children(): self.tree.delete(i)
        for it in issues: self.tree.insert("", "end", values=(f"{it['beat']:.2f}", it["type"], it["detail"]))
        if issues: self.status.set(f"编辑完成：{len(issues)} 个潜在问题")
        else: self.status.set("编辑完成 ✓")
        # 显示最新
        self.roll.set_score(sc_from_roll, sustain=sc_from_roll.sustain)
    ...
    def gen_chords_overwrite(self, append=False):
        ...
        self.roll.set_score(sc, sustain=sc.sustain); self.refresh_and_check()

    def pick_sf2(self):
        p=filedialog.asksaveasfilename() if False else filedialog.askopenfilename(filetypes=[("SoundFont","*.sf2"),("All files","*.*")])
