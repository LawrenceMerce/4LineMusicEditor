# SATB Arranger - Version 2.2 (integrated fixed)
# - 钢琴卷帘图（修复：不访问 self.scrollregion；颜色使用 #RRGGBB）
# - 规则检查（音域越界、声部交叉、并行五度/八度）
# - 三后端：SoundFont(FluidSynth)/Sampler(WAV)/System MIDI
# - 首次播放无需“先停止”
# 依赖：pygame midiutil numpy（推荐：pyfluidsynth）。Windows 用 SF2 时需能找到 FluidSynth DLL。
...
APP_TITLE = "SATB Arranger — v2.2 (Piano Roll + Rule Checker)"
DEFAULT_BPM = 90
DEFAULT_METER = (4, 4)  # 4/4
...
VOICE_COLOR = {"S":"%s"%("#f26b6b"), "A":"#f2a35e", "T":"#59c972", "B":"#5aa6f2"}

RANGES = {"S":(60,84), "A":(55,77), "T":(48,69), "B":(40,60)}  # 建议音域(C4=60)
...
class RuleChecker:
    def __init__(self, score: Score):
        self.sc = score
        self.issues = []  # list of dict: {beat, type, detail}

    def check(self):
        self.issues.clear()
        self.check_range()
        self.check_crossing()
        self.check_parallels()
        return self.issues
        ...
        for v in VOICE_ORDER:
            ...
            for n in self.sc.parts[v].notes:
                if n.pitch is not None and not (lo <= n.pitch <= hi):
                    self.issues.append({"beat": t, "type": "越界", "detail": f"{v} {format_pitch(n.pitch)} 不在建议音域[{lo}-{hi}]", "voice":v})
                t += n.beats
    ...
    def build_timeline(self):
        ...
        for v in VOICE_ORDER:
            t=0.0
            for n in self.sc.parts[v].notes:
                starts.add(t); t+=n.beats; starts.add(t)
        pts=sorted(starts)
        timeline=[]
        ws={v:self.sc.parts[v].with_starts() for v in VOICE_ORDER}
        for b in pts:
            snap={}
            for v in VOICE_ORDER:
                seq=ws[v]
                ...
                for st, n in seq:
                    if st<=b<st+n.beats:
                        ...
                        break
                snap[v]=curp
            timeline.append((b, snap))
        return timeline

    def check_crossing(self):
        tl = self.build_timeline()
        for b, snap in tl:
            s,a,t,bass = snap["S"], snap["A"], snap["T"], snap["B"]
            if None not in (s,a) and s < a:
                self.issues.append({"beat":b,"type":"交叉","detail":"S < A（声部交叉）","voice":"S/A"})
            if None not in (a,t) and a < t:
                self.issues.append({"beat":b,"type":"交叉","detail":"A < T（声部交叉）","voice":"A/T"})
            if None not in (t,bass) and t < bass:
                self.issues.append({"beat":b,"type":"交叉","detail":"T < B（声部交叉）","voice":"T/B"})
    ...
    def check_parallels(self):
        ...
        for i in range(1, len(tl)):
            ...
            b1,snap1=tl[i]
            for (u,v) in pairs:
                p0_u, p0_v = snap0[u], snap0[v]
                p1_u, p1_v = snap1[u], snap1[v]
                if None in (p0_u,p0_v,p1_u,p1_v): continue
                intv0 = abs(p0_u - p0_v)
                intv1 = abs(p1_u - p1_v)
                du = p1_u - p0_u
                dv = p1_v - p0_v
                same_dir = (du>0 and dv>0) or (du<0 and dv<0)
                moved = (du!=0 or dv!=0)
                if same_dir and moved:
                    if intv0 in (7, 19) and intv1 in (7, 19):
                        self.issues.append({"beat":b1,"type":"并行五度","detail":f"{u}-{v} 在 {b0:.2f}→{b1:.2f} 发生","voice":f"{u}/{v}"})
                    if intv0%12==0 and intv1%12==0:
                        self.issues.append({"beat":b1,"type":"并行八度","detail":f"{u}-{v} 在 {b0:.2f}→{b1:.2f} 发生","voice":f"{u}/{v}"})

# ====================== Piano Roll 视图（修复 + 合法颜色） ======================
class PianoRollCanvas(tk.Canvas):
    def __init__(self, master, **kw):
        ...
        self._cursor_start_ts = 0
        self._bpm = DEFAULT_BPM
        self._scroll_follow = True

        # 记录滚动区域宽高（替代 self.scrollregion）
        self._scroll_w = 0
        self._scroll_h = 0

        # 滚动条
        self.xscroll = ttk.Scrollbar(master, orient="horizontal", command=self.xview)
        self.configure(xscrollcommand=self.xscroll.set)
        self.xscroll.pack(fill="x", side="bottom")

    def set_score(self, score: Score):
        self.delete("all")
        self._bpm = score.bpm
        self.total_beats = score.total_beats()

        ps=[]
        for v in VOICE_ORDER:
            for t, n in score.parts[v].with_starts():
                if n.pitch is not None: ps.append(n.pitch)
        if ps:
            self.min_pitch = max(24, min(ps)-3)
            self.max_pitch = min(96, max(ps)+3)
        else:
            self.min_pitch, self.max_pitch = 36, 84

        w = self.margin_left + int(self.total_beats*self.beat_px) + 200
        h = self.margin_top + (self.max_pitch - self.min_pitch + 1)*self.pitch_px + 20

        self._scroll_w = w
        self._scroll_h = h
        self.config(scrollregion=(0,0,w,h), width=min(1000,w), height=300)

        self.draw_grid(score)
        self.draw_notes(score)

        x = self.margin_left
        if self.cursor:
            self.delete(self.cursor)
        # 颜色改为 6 位 #RRGGBB（浅黄）
        self.cursor = self.create_line(x, self.margin_top, x, h-10, fill="#ffffaa", width=2)

    def beat_to_x(self, beat):
        return self.margin_left + beat*self.beat_px

    def pitch_to_y(self, pitch):
        return self.margin_top + (self.max_pitch - pitch)*self.pitch_px

    def draw_grid(self, score: Score):
        num, den = score.meter
        total = int(np.ceil(self.total_beats))
        for b in range(total+1):
            ...
            if b%num==0:
                ...
                color = "#333846"
            self.create_line(x, self.margin_top, x, self._scroll_h-10, fill=color, width=w, tags="grid")
        for p in range(self.min_pitch, self.max_pitch+1):
            y = self.pitch_to_y(p)
            col = "#161a20" if p%12 else "#1b2028"
            self.create_line(self.margin_left, y, self._scroll_w, y, fill=col, width=1)
        for p in range(self.min_pitch, self.max_pitch+1):
            if p%12==0:
                y=self.pitch_to_y(p)
                self.create_text(20, y-6, text=f"C{p//12-1}", fill="#aaaaaa", font=("Consolas", 10))

    def draw_notes(self, score: Score):
        for v in VOICE_ORDER:
            color = VOICE_COLOR[v]
            ypad = {"S":0,"A":1,"T":2,"B":3}[v]*0.2
            t=0.0
            for n in score.parts[v].notes:
                if n.pitch is not None:
                    x0=self.beat_to_x(t)
                    x1=self.beat_to_x(t+n.beats)
                    y0=self.pitch_to_y(n.pitch)+ypad
                    y1=y0 + int(self.pitch_px*0.8)
                    self.create_rectangle(x0, y0, x1, y1, fill=color, outline="", tags=("note", v))
                t += n.beats

    def start_cursor(self, bpm, total_beats):
        self._bpm = bpm
        self.total_beats = total_beats
        self._cursor_start_ts = time.perf_counter()
    ...
    def _tick(self):
        ...
        elapsed = time.perf_counter() - self._cursor_start_ts
        cur_beat = elapsed / beats_to_seconds(1, self._bpm)
        x = self.beat_to_x(cur_beat)
        self.coords(self.cursor, x, self.margin_top, x, self._scroll_h-10)

        if self._scroll_follow:
            w_vis = self.winfo_width()
            denom = max(1, (self._scroll_w - w_vis))
            self.xview_moveto(max(0, (x - self.margin_left - w_vis*0.4) / denom))

        if cur_beat <= self.total_beats + 0.2:
    ...
    def scroll_to_beat(self, beat):
        ...
        w_vis = self.winfo_width()
        denom = max(1, (self._scroll_w - w_vis))
        self.xview_moveto(max(0, (x - self.margin_left - w_vis*0.5) / denom))
        self.coords(self.cursor, x, self.margin_top, x, self._scroll_h-10)
...
class App:
    def __init__(self, root):
        self.root=root
        root.title(APP_TITLE); root.geometry("1180x820")
        self._style()
    ...
    def _build_ui(self):
        ...
        ttk.Button(bar, text="刷新视图/检查", command=self.refresh_and_check).pack(side="left", padx=(16,6))
        ...
        gen = ttk.Labelframe(left, text="从和弦自动生成（基础声部连贯）")
        ...
        roll_box = ttk.Labelframe(right, text="钢琴卷帘图")
        roll_box.pack(fill="both", expand=True)
        self.roll = PianoRollCanvas(roll_box)
        self.roll.pack(fill="both", expand=True)

        issues_box = ttk.Labelframe(right, text="规则问题")
        issues_box.pack(fill="both", expand=False, pady=6)
        cols=("beat","type","detail")
        self.tree = ttk.Treeview(issues_box, columns=cols, show="headings", height=8)
        for c in cols:
            self.tree.heading(c, text=c)
        self.tree.column("beat", width=80, anchor="center")
        self.tree.column("type", width=100, anchor="center")
        self.tree.column("detail", width=640, anchor="w")
    ...
    def _collect_score(self):
        sc=Score(self.bpm.get(), DEFAULT_METER)
        for v in VOICE_ORDER:
    ...
    def _build_backend(self):
        ...
        if chosen.startswith("SoundFont"):
            ...
            if self.sample_dir.get():
                ...
                if n==0:
                    messagebox.showwarning("采样器","该目录未找到如 piano_C4.wav 的样本；将改为 System MIDI 兜底。")
            return be
        else:
    ...
    def play(self):
        ...
        # 卷帘展示 & 光标
        self.roll.set_score(sc)
        self.roll.start_cursor(sc.bpm, sc.total_beats())
    ...
    def export_midi(self):
        ...
        for i,v in enumerate(VOICE_ORDER):
            ...
            mf.addProgramChange(i, VOICE_TO_CH[v], 0, 0)  # 全钢琴
        for v in VOICE_ORDER:
            t=0.0
    ...
    def refresh_and_check(self):
        ...
        if issues:
            self.status.set(f"发现 {len(issues)} 个问题（点击列表可定位）")
        else:
            self.status.set("规则检查通过 ✓")

    def on_select_issue(self, _event):
        sel = self.tree.selection()
        if not sel: return
        beat_str = self.tree.item(sel[0])["values"][0]
        try:
            b = float(beat_str)
            self.roll.scroll_to_eat = getattr(self.roll, "scroll_to_beat", None)
            if self.roll.scroll_to_beat:
                self.roll.scroll_to_beat(b)
        except:
            pass
