# SATB Arranger - Version 3.3
# - 保留钢琴卷帘
# - SATB 文本支持并回写：力度 dyn/vel、踏板 ped=on/off
# - 新增：articulation（stacc/ten/accent/marcato）与 tie（start/cont/end）
# - 播放与导出 MIDI 时生效（含 CC64）
# 依赖：pygame midiutil numpy（可选：pyfluidsynth）
# Windows 使用 SoundFont 时，请确保 FluidSynth DLL 可用（设置 FLUIDSYNTH_BIN 或 C:\tools\fluidsynth\bin）
...
APP_TITLE   = "SATB Arranger — v3.3 (Roll + Note Attr + Articulation + Tie)"
DEFAULT_BPM = 90
DEFAULT_METER = (4, 4)
...
#      art=stacc|ten|accent|marcato （也支持 {stacc}/{ten}/{accent}/{marcato} 简写）
#      tie=start|cont|end           （也支持 {tie} 作为 start 简写）
def parse_token(tok: str):
    """
    返回: (pitch, beats, attr:dict) ；休止符 pitch=None
    """
    tok = tok.strip()
    ...
    # 解析 {...} 属性块
    attr = {}
    m_attr = re.search(r"\{([^}]*)\}\s*$", tok)
    if m_attr:
        body = m_attr.group(1).strip()
        tok  = tok[:m_attr.start()].strip()
        if body:
            for raw in body.split(","):
                ...
                if "=" in raw:
                    ...
                    if key == "ped":
                        attr["ped"] = "on"
                    elif key in ("stacc","staccato"):
                        attr["art"] = "stacc"
                    elif key in ("ten","tenuto"):
                        attr["art"] = "ten"
                    elif key == "accent":
                        attr["art"] = "accent"
                    elif key == "marcato":
    ...
    if m:
        pitch = note_to_midi(m.group(1)); base = DUR_MAP[m.group(2).lower()]
        dots = (m.group(3) or "")
        return (pitch, base*(1.5**len(dots)), attr)

    # 音符 + 数值时值
    m2 = re.fullmatch(r"([A-Ga-g][#b]?[-]?\d)(\d+(?:\.\d+)?)", tok)
    if m2: return (note_to_midi(m2.group(1)), float(m2.group(2)), attr)
...
# ====================== Piano Roll（同 v3.2，补齐 scroll_to_beat） ======================
class PianoRollCanvas(tk.Canvas):
    ...
    def scroll_to_beat(self, b):
        x = self.beat_to_x(b)
        wv = self.winfo_width()
        denom = max(1, (self._scroll_w - wv))
        self.xview_moveto(max(0, (x - self.margin_left - wv*0.5)/denom))
        main_h=self.margin_top+(self.max_pitch-self.min_pitch+1)*self.pitch_px+20
        if self.cursor:
            self.coords(self.cursor, x, self.margin_top, x, main_h-10)
...
class App:
    ...
    def _build_ui(self):
        ...
        ttk.Label(top, text="  卷帘编辑 · 音符属性(dyn/vel/ped) · 发音法/连音 · CC64 · 规则检查", style="Caption.TLabel").pack(side="left")
    ...
    def _load_demo(self):
        # —— Demo：含 dyn/vel/ped + articulation + tie ——（4 小节）
        demo_S = (
        ...
        demo_T = (
            "E4h E4h | "
            "F4q{stacc} G4q F4q E4q | "
            "D4h.{accent} C4q | "
            "B3q C4q D4q E4q"
        )
        demo_B = (
            "C3w | "
            "C3h G2h{marcato,vel=95} | "
            "F2w | "
            "C3w"
        )
        for v,txt in zip(VOICE_ORDER, [demo_S,demo_A,demo_T,demo_B]):
    ...
    def _score_from_texts(self, bpm=None):
        ...
        for v in VOICE_ORDER:
            ...
            for tok in toks:
                ...
                vel=96
                if "vel" in (attr or {}):
                    try: vel=int(clamp(int(attr["vel"]),1,127))
                    except: vel=96
                elif "dyn" in (attr or {}) and attr["dyn"] in DYN_TO_VEL:
    ...
    def _texts_from_score(self, sc:Score):
        ...
        for v in VOICE_ORDER:
            ...
            for st,n in ws:
                ...
                toks.append(f"{base}{{{','.join(attrs)}}}" if attrs else base)
    ...
    # ---------- 解析文本 -> 事件（含 articulation + tie） ----------
    def _parse_text_tokens_by_voice(self):
        tokens_by_voice = {}
        toggle_times = []  # (beat, 127/0)
        for v in VOICE_ORDER:
            text = self.texts[v].get("1.0","end").replace("|"," ")
            toks = [t for t in text.split() if t.strip()]
            t = 0.0
            ...
            for tok in toks:
                ...
                if "ped" in (attr or {}):
                    ...
                    toggle_times.append((t, val))
                t += b
            tokens_by_voice[v] = seq

        total = 0.0
        for v in VOICE_ORDER:
            total = max(total, sum(b for (_,b,_) in tokens_by_voice[v]))
        sustain = self._toggles_to_intervals(toggle_times, total)
        return tokens_by_voice, sustain, total

    def _events_from_texts_with_articulation(self):
        tokens_by_voice, sustain, total_beats = self._parse_text_tokens_by_voice()

        def vel_from_attr(attr):
            if not attr: return 96
            if "vel" in attr:
                try: return int(clamp(int(attr["vel"]),1,127))
                except: return 96
            if "dyn" in attr and attr["dyn"] in DYN_TO_VEL:
                return DYN_TO_VEL[attr["dyn"]]
            return 96

        def art_params(attr):
            """(len_factor, vel_factor)"""
            if not attr: return (0.98, 1.0)
            a = attr.get("art","")
            if a in ("stacc","staccato"):   return (0.55, 1.00)
            if a in ("ten","tenuto"):       return (0.98, 1.00)
            if a == "accent":               return (0.90, 1.20)
            if a == "marcato":              return (0.75, 1.25)
            return (0.98, 1.0)

        events = []
        note_specs = {v: [] for v in VOICE_ORDER}

        # 踏板 CC（全通道）
        for (s,e) in sustain:
            ts = beats_to_seconds(s, self.bpm.get())
            te = beats_to_seconds(e, self.bpm.get())
            for ch in range(4):
                events.append((ts, True, -1, ch, 127))
                events.append((te, True, -1, ch,   0))

        # 四声部生成（含 tie 合并）
        for v in VOICE_ORDER:
            ch = VOICE_TO_CH[v]
            seq = tokens_by_voice[v]
            t  = 0.0
            i  = 0
            while i < len(seq):
                p,b,attr = seq[i]
                ...
                tie_flag = (attr or {}).get("tie","")
                is_tie_start = (tie_flag in ("start","1","true","on")) or (tie_flag == "")
                chain_beats = b
                j = i
                if is_tie_start:
                    k = i + 1
                    while k < len(seq):
                        ...
                        if p2 != p:
                            break
                        flag2 = (attr2 or {}).get("tie","")
                        if flag2 in ("cont","end","1","true","on",""):
                            chain_beats += b2
                            j = k
                            if flag2 == "end":
                                break
                ...
                base_vel = vel_from_attr(attr)
                lf, vf   = art_params(attr)
                eff_len_factor = (0.98 if j > i else lf)   # tie 合并时不缩短
                vel_play = int(clamp(round(base_vel * vf), 1, 127))

                start_sec = beats_to_seconds(t, self.bpm.get())
                dur_sec   = beats_to_seconds(chain_beats, self.bpm.get()) * eff_len_factor

                pt = p + self.transpose.get()
                events.append((start_sec, True,  pt, ch, vel_play))
                events.append((start_sec + dur_sec, False, pt, ch, 0))
                note_specs[v].append((t, pt, chain_beats * eff_len_factor, vel_play))

                t += chain_beats
                i  = j + 1

        events.sort(key=lambda e:(e[0], 0 if not e[1] else 1))
        return events, note_specs, sustain, total_beats
    ...
    def play(self):
        self.stop()
        # 用“带发音法+连音”的事件构建器
        events, note_specs, sustain, total_beats = self._events_from_texts_with_articulation()
        # 同步卷帘显示（按 Score 渲染；art/tie 不在卷帘上画，仅在播放/导出生效）
        sc = self._score_from_texts(self.bpm.get())
        ...
        self.status.set(f"播放中（{self.bpm.get()} BPM, 移调 {self.transpose.get()}） | 后端: {be.name}")
    ...
    def export_midi(self):
        ...
        events, note_specs, sustain, _total = self._events_from_texts_with_articulation()

        mf = MIDIFile(5)  # 0: 全局(含踏板 CC)；1..4: S/A/T/B
        bpm = self.bpm.get()
        mf.addTempo(0, 0, bpm)
        # 踏板 CC 写到轨 0（通道 0）
        for (s,e) in sustain:
        ...
        # 四声部音符（使用 note_specs 的有效时长/力度）
        for i,v in enumerate(VOICE_ORDER, start=1):
            ch = VOICE_TO_CH[v]
            mf.addTempo(i, 0, bpm); mf.addProgramChange(i, ch, 0, 0)
            for (start_beat, pitch, eff_dur, vel) in note_specs[v]:
                mf.addNote(i, ch, pitch, start_beat, eff_dur, vel)
    ...
    def on_score_modified(self, sc_from_roll: Score):
        # 卷帘修改后：把力度/延音写回 SATB 文本（art/tie 暂不回写，下一版考虑持久化）
        self._current_score=sc_from_roll
        self._texts_from_score(sc_from_roll)
        self.refresh_and_check()
